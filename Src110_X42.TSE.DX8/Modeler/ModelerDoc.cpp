...
BOOL CModelerDoc::OnOpenDocument(LPCTSTR lpszPathName) 
{
  ...
  CTFileName fnModelFile = CTString(CString(lpszPathName));
...
BOOL CModelerDoc::OnSaveDocument(LPCTSTR lpszPathName) 
{
  ...
  while (pos!=NULL)
  {
    ...
    if( pmdCurrent != this)
    {
      ...
      {
        CTString strMessage;
        CTFileName fnDoc = CTString(CString(pmdCurrent->GetPathName()));
...
void CModelerDoc::OnFileAddTexture() 
{
  ...
  CTFileName fnDocName = CTString(CString(GetPathName()));
  theApp.WriteProfileString( "Scape", "Add texture directory", CString(fnDocName.FileDir()));
...
{
  CTFileName fnResult = CTFileName( CTString(CString(GetPathName()))).FileDir();
  return CTString( fnResult);
}

CTString CModelerDoc::GetModelName( void)
{
  CTFileName fnResult = CTFileName( CTString(CString(GetPathName()))).FileName();
  return CTString( fnResult);
}
