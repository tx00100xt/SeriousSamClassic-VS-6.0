...
void CBrowseWindow::OnContextMenu( CPoint point)
{
  ...
  if( pVTNDir == NULL)
  {
    AfxMessageBox( "Virtual tree doesn't yet exists. Create at least one virtual directory "
                   "to be able to insert items into it.");
    return;
  }
...
void CBrowseWindow::OnDropFiles(HDROP hDropInfo)
{
  ...
  {
    AfxMessageBox( "ERROR: Virtual tree doesn't exist.");
  }
...
void CBrowseWindow::OpenDirectory( CVirtualTreeNode *pVTNDir)
{
  ...
  char achrOpenedDirectoryMessage[ 256];
  sprintf( achrOpenedDirectoryMessage, "Opened directory: \"%s\"",
