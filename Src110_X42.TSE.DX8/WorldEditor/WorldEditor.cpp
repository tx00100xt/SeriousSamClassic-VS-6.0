...
#define INI_PRIMITIVE_READ( strname, default_val)                               \
  strcpy( strIni, CString(theApp.GetProfileString( "World editor prefs", CString(strPrimitiveType+" "+ strname ), CString( default_val ))))
#define INI_PRIMITIVE_WRITE( strname)                                   \
  theApp.WriteProfileString( "World editor prefs", CString(strPrimitiveType+" "+strname), CString(strIni))

#define INI_READ( strname, default_val)                               \
  strcpy( strIni, CString(theApp.GetProfileString( "World editor prefs", CString( strname ), CString(default_val))))
#define GET_FLAG( var)                                        \
...
#define INI_WRITE( strname)                                   \
  theApp.WriteProfileString( "World editor prefs", CString( strname ), CString(strIni))
...
void CWorldEditorApp::MyParseCommandLine(void)
{
  ...
  FOREVER {
    ...
    if (strWord=="") {
      ...
      m_lpCmdLine = (LPSTR)(LPCSTR)_strCmdW;
      return;
...
BOOL CWorldEditorApp::SubInitInstance()
{
  ...
  InitCommonControls();
	...
	if (!AfxOleInit())
	{
    AfxMessageBox("ERROR: Failed to initialize OLE 2.0 libraries");
		return FALSE;
	}
  ...
  CString strDefaultTexture = GetProfileString( "World editor prefs", "Default primitive texture", "");
  if (strDefaultTexture == "") {
...
/*    _spawnlp(_P_WAIT, "regedit.exe", 
  ...
  m_iApi=GAT_OGL;
  m_iApi=AfxGetApp()->GetProfileInt("Display modes", "SED Gfx API", GAT_OGL);
  ...
  CTString strCmdLine=CTString(CString(m_lpCmdLine));
...
void CWorldEditorApp::ReadFromIniFileOnInit(void)
{
  ...
  CString strTexture = GetProfileString( "World editor prefs", "Default primitive texture", "Textures\\Editor\\Default.tex");
  // if exists in ini file
  if( strTexture != "")
  {
    theApp.SetNewActiveTexture( _fnmApplicationPath + CTString(CString(strTexture)));
  }
  ...
  m_bShowTipOfTheDay = GetProfileInt("World editor", "Show Tip of the Day", TRUE);
  m_iCurrentTipOfTheDay = GetProfileInt("World editor", "Current Tip of the Day", 0);
...
void CWorldEditorApp::WriteToIniFileOnEnd(void)
{
  ...
  if( theApp.m_ptdActiveTexture != NULL)
  {
    ...
    WriteProfileString( "World editor prefs", "Default primitive texture", CString(strIni));
  }
  ...
  WriteProfileInt("World editor", "Show Tip of the Day", m_bShowTipOfTheDay);
  WriteProfileInt("World editor", "Current Tip of the Day", m_iCurrentTipOfTheDay);
...
void CWorldEditorApp::OnConvertWorlds()
{
  ...
  dlgProgressDialog.SetWindowText( "Convert files");
...
void CWorldEditorApp::OnFileNew()
{
  ...
  CTFileName fnDefaultBcg = CTString(CString(theApp.GetProfileString(
    "World editor prefs", "Default background picture",
...
//!!!!    pDoc->m_woWorld.SetBackgroundTexture_t( CTString("Textures\\Editor\\Default.tex"));
  ...
  strcpy( chrColor, CString(theApp.GetProfileString( "World editor prefs",
                                             "Default background color", "0XFF000000")));
...
void CWorldEditorApp::DisplayHelp(const CTFileName &fnHlk, UINT uCommand, DWORD dwData)
{
  ...
  if( bHlkFound)
  {
    if( strHelpFormatID=="HTM" || strHelpFormatID=="HTM" || strHelpFormatID=="TXT")
    {
