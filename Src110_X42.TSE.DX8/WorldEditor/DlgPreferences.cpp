...
void CDlgPreferences::DoDataExchange(CDataExchange* pDX)
{
  ...
  if( pDX->m_bSaveAndValidate != FALSE)
  {
    ...
    theApp.m_Preferences.ap_strSourceSafeProject = CString(m_strSSProject);
...
BOOL CDlgPreferences::OnInitDialog()
{
  ...
  {
    INDEX iAddedAs=m_ctrGfxApi.AddString("OpenG");
    m_ctrGfxApi.SetItemData(iAddedAs, GAT_OGL);
  }
#ifdef SE1_D3D
  if( _pGfx->HasAPI( GAT_D3D))
  {
    INDEX iAddedAs=m_ctrGfxApi.AddString("Direct3D");
    m_ctrGfxApi.SetItemData(iAddedAs, GAT_D3D);
  }
#endif // SE1_D3D

  m_ctrlTerrainSelectionVisible.ResetContent();
  m_ctrlTerrainSelectionVisible.AddString("Texture");
  m_ctrlTerrainSelectionVisible.AddString("Wireframe");
  m_ctrlTerrainSelectionVisible.AddString("Vertices");
  m_ctrlTerrainSelectionVisible.AddString("None");

  m_ctrlTerrainSelectionHidden.ResetContent();
  m_ctrlTerrainSelectionHidden.AddString("Texture");
  m_ctrlTerrainSelectionHidden.AddString("Wireframe");
  m_ctrlTerrainSelectionHidden.AddString("Vertices");
  m_ctrlTerrainSelectionHidden.AddString("None");
