...
void CDlgSnapVertex::DoDataExchange(CDataExchange* pDX)
{
  ...
  if( pDX->m_bSaveAndValidate == FALSE)
  {
    ...
    if( !bValidX) GetDlgItem(IDC_VTX_SNAP_X)->SetWindowText("");
    if( !bValidY) GetDlgItem(IDC_VTX_SNAP_Y)->SetWindowText("");
    if( !bValidZ) GetDlgItem(IDC_VTX_SNAP_Z)->SetWindowText("");
  }
