	...
	: CDialog(CDlgLightAnimationEditor::IDD, pParent)
{
  ...
  try
  {
    CTFileName fnLastEditted = CTString( CString(theApp.GetProfileString("World editor", "Last edited light animation", CString(DEFAULT_ANIMATION_FILE))));
...
{
  theApp.WriteProfileString("World editor", "Last edited light animation", CString(m_padAnimData->GetName()));
...
void CDlgLightAnimationEditor::StoreData(void)
{
  ...
  m_padAnimData->SetName( iLightAnimation, CTString(CString(m_strLightAnimationName)));
