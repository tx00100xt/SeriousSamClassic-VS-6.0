    ...
    if( IsWindow( _wndThumbnail) )
    {
      ...
      if( (_pDrawPort != NULL) && (_pDrawPort->Lock()) )
      {
        ...
        else
        {
          ...
          CWnd::FromHandle( GetDlgItem( hdlg, IDC_THUMBNAIL_DESCRIPTION))->SetWindowText( "No thumbnail");		  
        }
        // unlock the drawport
        _pDrawPort->Unlock();
      }
        ...
        BOOL bIfOpen/*=TRUE*/)
{
  ...
  if( pchrRegistry != NULL)
  {
    ...
    strRequestInDirectory = AfxGetApp()->GetProfileString("Scape", CString(pchrRegistry), 
      CString(_fnmApplicationPath+strDefaultDir));	
  }
  ...
  if( bResult)
  {
    ...
    if( pafnSelectedFiles != NULL)
    {
      ...
      if( pchrRegistry != NULL)
      {
		...
		AfxGetApp()->WriteProfileString("Scape", CString(pchrRegistry), CString(chrFiles));
      }
    ...
    else
    {
      ...
      if( pchrRegistry != NULL)
      {
		...
		AfxGetApp()->WriteProfileString("Scape", CString(pchrRegistry), strChooseFilePath);
      }
