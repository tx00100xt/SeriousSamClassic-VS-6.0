...
void CGfxLibrary::InitAPIs(void)
{
  ...
  for( INDEX iAdapter=0; iAdapter<ctMaxAdapters; iAdapter++)
  {
    ...
    pda->da_ulFlags = NONE;
    pda->da_ctDisplayModes = 0;
    INDEX ctModes = gl_pD3D->GetAdapterModeCount(iAdapter, D3DFMT_A8R8G8B8); // ### DX9
    INDEX iMode;
    HRESULT hr;
    ...
    D3DCAPS9 d3dCaps; // ### DX9
    gl_pD3D->GetDeviceCaps( iAdapter, d3dDevType, &d3dCaps);

    //if( !(d3dCaps.Caps2 & D3DCAPS2_CANRENDERWINDOWED)) pda->da_ulFlags |= DAF_FULLSCREENONLY; // ### DX9 //
    _pGfx->gl_pd3dCaps = d3dCaps; // ### DX9 for test support vertex shader (i.e. program)
    ...
    for( iResolution=0; iResolution<MAX_RESOLUTIONS; iResolution++)
    {
      ...
      for( iMode=0; iMode<ctModes; iMode++) {
        ...
        gl_pD3D->EnumAdapterModes( iAdapter, D3DFMT_A8R8G8B8, iMode, &d3dDisplayMode); // ### DX9
    ...
    gl_pD3D->GetAdapterIdentifier( iAdapter, 0, &d3dAdapterIdentifier);
...
////#endif
  //if( gl_hiDriver!=NONE) FreeLibrary((HMODULE)gl_hiDriver); // ### DX9 //
  gl_hiDriver = NONE;
#endif // SE1_D3D
}
