...
// TEXTURE MANAGEMENT
#ifdef SE1_D3D
static LPDIRECT3DTEXTURE9 *_ppd3dCurrentTexture;
#endif // SE1_D3D
...
extern void  UploadTexture_D3D( LPDIRECT3DTEXTURE9 *ppd3dTexture, ULONG *pulTexture,
...
{ 
  ...
  { // just set it
    ...
    for( INDEX iUnit=0; iUnit<_pGfx->gl_ctTextureUnits; iUnit++) { // loop thru tex units
	  // ####
	  // https://learn.microsoft.com/en-us/windows/win32/direct3d9/converting-to-directx-9
	  // https://learn.microsoft.com/en-us/windows/win32/api/d3d9/nf-d3d9-idirect3ddevice9-setsamplerstate
	  hr = _pGfx->gl_pd3dDevice->SetSamplerState(iUnit, D3DSAMP_MIPMAPLODBIAS, *((DWORD*)&fLODBias));
...
{              
 ...
 _D3DTEXTUREFILTERTYPE eMagFilter, eMinFilter, eMipFilter;
  ...
  for( INDEX iUnit=0; iUnit<_pGfx->gl_ctTextureUnits; iUnit++) { // must loop thru all usable texture units
	...
	hr = pd3dDev->SetSamplerState( iUnit, D3DSAMP_MAXANISOTROPY, iAnisotropyDegree);  D3D_CHECKERROR(hr);
...
extern void gfxSetTexture( ULONG &ulTexObject, CTexParams &tpLocal)
{
  ...
  if( eAPI==GAT_OGL) { // OpenGL
    ...
    _ppd3dCurrentTexture = (LPDIRECT3DTEXTURE9*)&ulTexObject;
...
extern void gfxUploadTexture( ULONG *pulTexture, PIX pixWidth, PIX pixHeight, ULONG ulFormat, BOOL bNoDiscard)
{
  ...
  if( eAPI==GAT_OGL) { // OpenGL
    ...
    const LPDIRECT3DTEXTURE9 _pd3dLastTexture = *_ppd3dCurrentTexture;
...
extern SLONG gfxGetTextureSize( ULONG ulTexObject, BOOL bHasMipmaps/*=TRUE*/)
{
  ...
  else if( eAPI==GAT_D3D)
  {
    ...
    D3D_CHECKERROR(hr);
	UINT iFormat = 4; // D3DFMT_X8B8G8R8
	switch (d3dSurfDesc.Format) {
		...
		case D3DFMT_A32B32G32R32F:
			...
			break;
		default:
			iFormat = 4; // D3DFMT_X8B8G8R8
			break;
	}
    slMipSize = d3dSurfDesc.Height*d3dSurfDesc.Width*iFormat;
	// #### old => slMipSize = d3dSurfDesc.Size;
  }
#endif // SE1_D3D
...
#ifdef SE1_D3D
  else if( eAPI==GAT_D3D) return GetTexturePixRatio_D3D( (LPDIRECT3DTEXTURE9)ulTextureObject);
#endif // SE1_D3D
  else return 0;
}
...
#ifdef SE1_D3D
  else if( eAPI==GAT_D3D && GFX_bTruform) { // ########?????? zapteritit GFX_bTruform ????
    // ########?????? zapreshaem
    truform_iLevel  = 0;
    truform_bLinear = FALSE;
    return;
