...
static void CreateSwapChain_D3D( CViewPort *pvp, PIX pixSizeI, PIX pixSizeJ)
{
  ...
  // ### DX9
  // Disable V-Sync
  d3dPresentParams.PresentationInterval = D3DPRESENT_INTERVAL_IMMEDIATE; // ### DX9
  ...
  hr = _pGfx->gl_pd3dDevice->CreateDepthStencilSurface( pixSizeI, pixSizeJ, _pGfx->gl_d3dDepthFormat,
                                                        D3DMULTISAMPLE_NONE, 0, FALSE, &pvp->vp_pSurfDepth, NULL); // ### DX9
  D3D_CHECKERROR(hr);
...
static void SetAsRenderTarget_D3D( CViewPort *pvp)
{
  ...
  //D3D_CHECKERROR(hr);
  hr = _pGfx->gl_pd3dDevice->SetDepthStencilSurface(pvp->vp_pSurfDepth); 
  D3D_CHECKERROR(hr);
  hr = _pGfx->gl_pd3dDevice->SetRenderTarget(0, pColorSurface); 
  D3D_CHECKERROR(hr);
  // ### DX9 end
...
void CViewPort::CloseCanvas( BOOL bRelease/*=FALSE*/)
{
  ...
  if( _pGfx->gl_eCurrentAPI==GAT_D3D && bRelease) {
    ...
    _pGfx->gl_pd3dDevice->GetRenderTarget(0, &pBackSurface);
