...
#define INI_READ( strname, def)                               \
  strcpy( strIni, theApp.GetProfileString( "Modeler prefs", CString(strname), CString(def)))
#define GET_FLAG( var)                                        \
...
#define INI_WRITE( strname)                                   \
  theApp.WriteProfileString( "Modeler prefs", CString(strname), strIni)
...
BOOL GetFlagFromProfile( CTString strVarName, BOOL bDefault)
{
  ...
  CTString strTemp = CString(theApp.GetProfileString( "Modeler prefs", CString(strVarName), CString(strDefault)));
...
{
  if( bValue) theApp.WriteProfileString( "Modeler prefs", CString(strVarName), "YES");
  else        theApp.WriteProfileString( "Modeler prefs", CString(strVarName), "NO");
};
...
void SetIndexToProfile( CTString strVarName, INDEX iValue)
{
  ...
  theApp.WriteProfileString( "Modeler prefs", CString(strVarName), CString(strTemp));
};
...
BOOL CModelerApp::SubInitInstance()
{
  ...
  m_iApi=GAT_OGL;
  m_iApi=GetProfileInt("Display modes", "SED Gfx API", GAT_OGL);
  ...
  if( iWorkingTexturesCt != -1) {
    ...
    for( INDEX i=0; i<iWorkingTexturesCt; i++) {
      ...
      AddModelerWorkingTexture( CTString(CString(strIni)));
    }
  }
  ...
  for( INDEX i=0; i<iWorkingPatchesCt; i++) {
    ...
    AddModelerWorkingPatch( CTString(CString(strIni)));
  }
...
void CAppPrefs::ReadFromIniFile()
{
  ...
  ap_DefaultWinBcgTexture = CTString(CString(strIni));
...
void CAppPrefs::WriteToIniFile()
{
  ...
  strcpy( strIni, CString(ap_DefaultWinBcgTexture));
  ...
  FOREACHINLIST( CBcgTexture, wt_ListNode, theApp.m_WorkingTextures, it_wt)
  {
    ...
    theApp.WriteProfileString( "Modeler prefs", CString(strWTName), CString(it_wt->wt_FileName));
    iWTCt++;
  }
  ...
  FOREACHINLIST( CWorkingPatch, wp_ListNode, theApp.m_WorkingPatches, it_wp)
  {
    ...
    theApp.WriteProfileString( "Modeler prefs", CString(strWPName), CString(it_wp->wp_FileName));
    iWPCt++;
  }
}
