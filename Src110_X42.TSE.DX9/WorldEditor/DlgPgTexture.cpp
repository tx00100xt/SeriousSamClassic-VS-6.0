...
void CDlgPgTexture::DoDataExchange(CDataExchange* pDX)
{
  ...
  if( (pDX->m_bSaveAndValidate != FALSE) && IsWindow( m_comboScroll.m_hWnd) )
  {
        ...
        (m_strTextureFile != NO_SELECTION) )
    {
      ...
      {
        pDoc->PasteTextureOverSelection_t( CTString( CString(m_strTextureFile)));
      }
    }
  ...
  m_wndViewTexture.m_strTexture = CString(strWindowText);
...
{
  GetDlgItem( IDC_TEXTURE_FILE_T)->SetWindowText( "No texture");
  // apply data change
	UpdateData( TRUE);
}
