...
void CDlgAutTexturize::DoDataExchange(CDataExchange* pDX)
{
  ...
  if( pDX->m_bSaveAndValidate == FALSE)
  {
    ...
    colOld=AfxGetApp()->GetProfileInt( "World editor", "Pretender bcg color", colOld);
    m_bExpandEdges=TRUE;
    m_bExpandEdges=AfxGetApp()->GetProfileInt( "World editor", "Auto expand edges", m_bExpandEdges);
  ...
  if( pDX->m_bSaveAndValidate != FALSE)
  {
    ...
    AfxGetApp()->WriteProfileInt( "World editor", "Pretender bcg color", m_colBcg.GetColor());
    AfxGetApp()->WriteProfileInt( "World editor", "Pretender resolution", iSelected);
    AfxGetApp()->WriteProfileInt( "World editor", "Auto expand edges", m_bExpandEdges);
    
    m_iPretenderStyle=m_ctrlPretenderTextureStyle.GetCurSel();
    AfxGetApp()->WriteProfileInt( "World editor", "Pretender style", m_iPretenderStyle);
  }
}
...
BOOL CDlgAutTexturize::OnInitDialog() 
{
  ...
  INDEX iSelected=6;
  iSelected=AfxGetApp()->GetProfileInt( "World editor", "Pretender resolution", iSelected);
  m_ctrPretenderTextureSize.SetCurSel(iSelected);

	m_ctrlPretenderTextureStyle.ResetContent();
  m_ctrlPretenderTextureStyle.AddString( "Front view only");
  m_ctrlPretenderTextureStyle.AddString( "Cylindrical view (FRBL)");
  m_ctrlPretenderTextureStyle.AddString( "Boxed view (FRBLUD)");
  iSelected=1;
  iSelected=AfxGetApp()->GetProfileInt( "World editor", "Pretender style", iSelected);
