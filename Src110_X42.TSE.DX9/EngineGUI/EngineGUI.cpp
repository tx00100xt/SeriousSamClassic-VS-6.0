...
void CEngineGUI::GetFullScreenModeFromRegistry( CTString strSectionName, CDisplayMode &dm, GfxAPIType &gat)
{
  ...
  CTString strResult = CString(AfxGetApp()->GetProfileString( CString(strSectionName), "Full screen mode", "640 x 480 x 0"));
  strResult.ScanF( "%d x %d x %d", &dm.dm_pixSizeI, &dm.dm_pixSizeJ, &dm.dm_ddDepth);
  if( dm.dm_ddDepth<DD_DEFAULT || dm.dm_ddDepth>DD_32BIT) dm.dm_ddDepth = DD_DEFAULT;
  strResult = CString(AfxGetApp()->GetProfileString( CString(strSectionName), "Full screen API", "OpenGL"));
#ifdef SE1_D3D
...
void CEngineGUI::SetFullScreenModeToRegistry( CTString strSectionName, CDisplayMode dm, GfxAPIType gat)
{
  ...
  AfxGetApp()->WriteProfileString(CString(strSectionName), "Full screen mode", CString(strDM));
  AfxGetApp()->WriteProfileString(CString(strSectionName), "Full screen API", CString(strGAT));
}
