...
void CBrowser::AddDirectoryRecursiv(CVirtualTreeNode *pOneDirectory, HTREEITEM hParent)
{
  ...
  InsertedDir = m_TreeCtrl.InsertItem( 0, "", 0, 0, TVIS_SELECTED, TVIF_STATE, 0,
                                       hParent, TVI_SORT );
...
void CBrowser::OnCreateDirectory() 
{
  ...
  if( dlg.DoModal() == IDOK)
  {
    ...
    if( m_TreeCtrl.GetCount() == 0)
    {
      ...
      m_VirtualTree.vtn_strName = CTString( CString(dlg.m_strDirectoryName));
    ...
    else
    {
      ...
      pvtnNewDir->vtn_strName = CTString( CString(dlg.m_strDirectoryName));
...
INDEX CBrowser::GetSelectedDirectory( CTString strArray[])
{
  ...
  FOREVER
  {
    CTString strItemName = CTString( CString(m_TreeCtrl.GetItemText(pItem)));
...
void CBrowser::OnRenameDirectory() 
{
  if( m_TreeCtrl.GetCount() != 0)
  {
    ...
    if( dlg.DoModal() == IDOK)
    {
      ...
      pVTN->vtn_strName = CTString( CString(dlg.m_strDirectoryName));
