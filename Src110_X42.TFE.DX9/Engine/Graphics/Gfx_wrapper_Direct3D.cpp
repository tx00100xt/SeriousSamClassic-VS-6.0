...
static void d3d_EnableDepthBias(void)
{
  ...
  // #### https://github.com/mattholden/Neverwood/blob/master/Direct3D.h#L274
  #ifndef D3DRS_ZBIAS
...
static void d3d_EnableTruform(void)
{
  ...
  // #### Disable
  // https://stackoverflow.com/questions/70414170/deletevertexshader-dx8-1-to-dx9-conversion
  /*
...
static void d3d_SetTextureWrapping( enum GfxWrap eWrapU, enum GfxWrap eWrapV)
{
  ...
  if( _tpGlobal[GFX_iActiveTexUnit].tp_eWrapU!=eWrapU) {
	...
	// #### https://learn.microsoft.com/en-us/windows/win32/direct3d9/converting-to-directx-9#setsamplerstate-changes
    //hr = _pGfx->gl_pd3dDevice->SetTextureStageState( GFX_iActiveTexUnit, D3DTSS_ADDRESSU, d3dta);
	hr = _pGfx->gl_pd3dDevice->SetSamplerState(GFX_iActiveTexUnit, D3DSAMP_ADDRESSU, d3dta);
  ...
  if( _tpGlobal[GFX_iActiveTexUnit].tp_eWrapV!=eWrapV) {
	...
	hr = _pGfx->gl_pd3dDevice->SetSamplerState(GFX_iActiveTexUnit, D3DSAMP_ADDRESSV, d3dta);
...
static void d3d_GenerateTexture( ULONG &ulTexObject)
{
  ...
  HRESULT hr = _pGfx->gl_pd3dDevice->CreateTexture( 1, 1, 0, 0, (D3DFORMAT)TS.ts_tfRGBA8, D3DPOOL_MANAGED, (LPDIRECT3DTEXTURE9*)&ulTexObject , /* ####[in] pSharedHandle Reserved */ NULL);
  D3D_CHECKERROR(hr);
...
static void d3d_Finish(void)
{
  ...
  if( bFullScreen)   hr = _pGfx->gl_pd3dDevice->GetBackBuffer( /* ###[in] UINT iSwapChain */0, 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer);
...
// #### https://learn.microsoft.com/en-us/previous-versions/ms889728(v=msdn.10)
// #### https://learn.microsoft.com/en-us/windows/win32/api/d3d9helper/nf-d3d9helper-idirect3ddevice9-setvertexshader
// #### https://learn.microsoft.com/en-us/windows/win32/api/d3d9helper/nf-d3d9helper-idirect3ddevice9-setfvf
// dwHandle forever D3DFVF_CTVERTEX. see "grep -r d3dSetVertexShade" and "grep -r D3DFVF_CTVERTEX"
...
extern void d3dSetVertexShader( DWORD  dwHandle)
{
  ...
  HRESULT hr = _pGfx->gl_pd3dDevice->SetFVF(dwHandle);
  D3D_CHECKERROR(hr);
