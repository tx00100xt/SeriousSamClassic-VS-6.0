...
#ifdef SE1_D3D
ULONG CGfxLibrary::SaveVertexShader(LPDIRECT3DVERTEXSHADER9 vs, LPDIRECT3DVERTEXDECLARATION9 vDec, ULONG ulStreamFlags)
{
	curVS++;
	/*
	stVertexShaderInfo* pInfo = new stVertexShaderInfo();
	pInfo->vertexShader = vs;
	pInfo->vertexDeclaration = vDec;
	pInfo->ulStreamFlags = ulStreamFlags;
	pInfo->ulHandle = curVS;
	pInfo->fvf = 0;

	mapVS.insert(std::pair<ULONG, stVertexShaderInfo*>(curVS, pInfo));
	*/

	_vs[curVS] = new stVertexShaderInfo();
	_vs[curVS]->vertexShader = vs;
	_vs[curVS]->vertexDeclaration = vDec;
	_vs[curVS]->ulStreamFlags = ulStreamFlags;
	_vs[curVS]->ulHandle = curVS;
	_vs[curVS]->fvf = 0;

	return curVS;
}

stVertexShaderInfo* CGfxLibrary::GetVertexShader(ULONG ulHandle)
{
	if (ulHandle <= curVS && ulHandle > 0)
		return _vs[ulHandle];


	/*
	auto it = mapVS.find(ulHandle);
	if (it != mapVS.end())
		return it->second;

	char tmp[128];
	sprintf(tmp, "Cannot load Shader handle %d", ulHandle);
	MessageBox(NULL, tmp, "Error", MB_OK);
	*/
	return NULL;
}

void CGfxLibrary::DeleteVertexShader(ULONG ulHandle)
{
	stVertexShaderInfo* pInfo = GetVertexShader(ulHandle);
	if (pInfo)
	{
		if (pInfo->vertexDeclaration)
			pInfo->vertexDeclaration->Release();

		if (pInfo->vertexShader)
			pInfo->vertexShader->Release();

		delete pInfo;
		//mapVS.erase(ulHandle);
	}
}
#endif // SE1_D3D
// ### DX9 end
...
#ifdef SE1_D3D
	curVS = 0;
	m_defaultDeclaration = NULL;
	m_defaultProgrammable = NULL;
	m_defaultFixed = NULL;
...
BOOL CGfxLibrary::LockDrawPort( CDrawPort *pdpToLock)
{
  ...
  { 
    ...
    D3DVIEWPORT9 d3dViewPort = { pixMinSI, pixMinSJ, pixMaxSI-pixMinSI+1, pixMaxSJ-pixMinSJ+1, 0,1 };
...
void CGfxLibrary::SwapBuffers(CViewPort *pvp)
{
  ...
  else if( gl_eCurrentAPI==GAT_D3D)
  {
    ...
    if( dm.dm_pixSizeI==0 || dm.dm_pixSizeJ==0 ) {
      // windowed mode
      hr = pvp->vp_pSwapChain->Present( NULL, NULL, NULL, NULL , /* ####[in] DWORD dwFlags */ 0 );
	  // If dwFlags = 0, this method behaves as it did prior to Direct3D 9. 
	  // Present will spin until the hardware is free, without returning an error.
    } else {
      // full screen mode
  ...
  if( gl_ulFlags & GLF_ADJUSTABLEGAMMA) {
    ...
    if( bTableSet) {
      if( gl_eCurrentAPI==GAT_OGL) {
        ...
        gl_pd3dDevice->SetGammaRamp(/* #### [in] UINT iSwapChain */ 0, D3DSGR_NO_CALIBRATION, (D3DGAMMARAMP*)&_auwGammaTable[0]);
      }
#endif // SE1_D3D
    }
  }
