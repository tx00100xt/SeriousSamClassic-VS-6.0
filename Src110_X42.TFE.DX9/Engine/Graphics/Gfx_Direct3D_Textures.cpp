...
extern void MimicTexParams_D3D( CTexParams &tpLocal)
{
  ...
  { 
    ...
    if( tpLocal.tp_bSingleMipmap) {
...
#endif // set it
	  // #### GFX_iActiveTexUnit -> Gfx_wrapper.cpp[325]
	  ...
	  hr = _pGfx->gl_pd3dDevice->SetSamplerState( GFX_iActiveTexUnit , D3DSAMP_MIPFILTER, D3DTEXF_NONE);
    ...
    // yes mipmaps?
	  ...
	  // https://learn.microsoft.com/en-us/windows/win32/direct3d9/converting-to-directx-9
	  // https://learn.microsoft.com/en-us/windows/win32/api/d3d9/nf-d3d9-idirect3ddevice9-setsamplerstate
	  hr = _pGfx->gl_pd3dDevice->SetSamplerState( GFX_iActiveTexUnit , D3DSAMP_MIPFILTER, eMipFilter);
                               ...
                               PIX pixSizeU, PIX pixSizeV, D3DFORMAT eInternalFormat, BOOL bDiscard)
{
  ...
  if( bDiscard) {
    ...
    hr = _pGfx->gl_pd3dDevice->CreateTexture( pixSizeU, pixSizeV, 0, 0, eInternalFormat, D3DPOOL_MANAGED, ppd3dTexture, /* #### [in] pSharedHandle  Type : HANDLE*  Reserved.Set this parameter to NULL. */ NULL);
    D3D_CHECKERROR(hr);
  }
